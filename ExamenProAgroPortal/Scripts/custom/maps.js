var idUser = parseInt($("#idUsuario").val(),10);
...
$.ajax({
    type: "POST",
    url: "http://localhost:58128/api/proagro/obtenergeorreferencia",
    data: modelo,
    ...
    success: function (source) {
        ...
        $.each(source.data, function (i, item) {
            json = json + "'{\"" + item.NombreEstado + "\":" + "{\" Position\"" + ":{" + "\"Longitude\":" + item.Longitud + "\"Latitude\":" + item.Latitud + "}},";
            jsonn = jsonn + '"{\"' + item.NombreEstado + '\":' + '{\" Position\"' + ':{' + '\"Longitude\":' + item.Longitud + '\"Latitude\":' + item.Latitud + '}},';
        });
        json = json.substring(0, json.length - 1);
        json = json + '}';

        jsonn = jsonn.substring(0, jsonn.length - 1);
        jsonn = jsonn + '}';

        var m = JSON.parse(jsonn);

        var myDatas = JSON.stringify(json);
        console.log(myDatas);
    },
